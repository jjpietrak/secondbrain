...
Indexes the `contextualized_text` field of every chunk under <vault>/.vault-meta/chunks/,
emits a single JSON file at <vault>/.vault-meta/bm25/index.json with the schema below.

Second Brain v0.2 port of the claude-obsidian retrieval pipeline. The ONLY adaptation
vs the CO original is the vault-root resolution (see _resolve_vault_root): the index and
chunk caches live under the ACTIVE VAULT's .vault-meta/ (resolved via agents.vault_config),
not under the code repo. The test-sandbox contract (scripts copied into <tmp>/scripts/
resolve their vault root to <tmp> via the __file__.parent.parent fallback) is preserved,
so the shipped hermetic tests stay unchanged.

Concurrency:
- Locks <vault>/.vault-meta/.bm25.lock (fcntl exclusive) around any index write.
...
Build interface:
  bm25-index.py build               # full rebuild (always; incremental is later scope)
...
def _resolve_vault_root():
    """Resolve the active vault root for .vault-meta storage.
    ...
    Second Brain adaptation (the one deviation from the CO original). Precedence:
      ...
      3. `agents.vault_config path` - resolve the active vault on demand, but ONLY
         when this script is running from its canonical repo location (so the
         hermetic subprocess tests, which copy the script into a tmp sandbox, fall
         through to the parent.parent fallback instead of pointing at the live vault)
      4. Path(__file__).resolve().parent.parent - the CO original's behavior, which
         is exactly what the shipped sandbox tests rely on.
    """
...
# Unicode-aware tokenizer. \w under re.UNICODE matches letters and digits from any
# script (CJK, Cyrillic, accented Latin, Devanagari, etc.) plus underscore. Internal
# apostrophes and hyphens are preserved so "user's" and "well-formed" stay single
# tokens. Pure-symbol or pure-emoji tokens fail the leading \w anchor and are skipped.
...
def query(text, top_k=20):
    ...
    # Defensive guard: avg_dl can only be 0 if the vocab is also empty (all chunks
    # have zero tokens), in which case the loop never enters this divide path. But
    # future refactors could change that invariant; the `or 1.0` keeps it safe.
...
def main():
    ...
    sub.add_parser("build", help="Build the index (full rebuild every time).")

...
Strategy (in preference order, automatically chosen at runtime):
  1. If ollama is reachable AND nomic-embed-text is pulled
       -> embed the query, embed each candidate's contextualized_text,
         ...
         <vault>/.vault-meta/embed-cache.json keyed by body_hash.
  2. Otherwise
...
Localhost-only OLLAMA_URL guard: remote ollama endpoints require
--allow-remote-ollama because page bodies are POSTed as embedding input. This is
the Second Brain $0/local default (Ollama `bulk` route on 127.0.0.1); the only
egress is double-consent-gated.

Second Brain adaptation vs the CO original: the ONLY change is vault-root
resolution (see _resolve_vault_root) so the embed cache and chunk files are read
under the ACTIVE VAULT's .vault-meta/. The sandbox-test contract is preserved.
...
def _resolve_vault_root():
    """Resolve the active vault root (embed cache + chunk files live under it).
...
def save_cache(cache):
    ...
    Uses LOCK_NB with a 3-attempt retry loop, then falls back to writing without
    the lock (with a WARN) so the rerank pipeline never hangs the user's session
    on a non-flock-capable filesystem (some NFS mounts, network shares, FUSE
    backends without lock support). The temp + os.replace pattern provides write
    atomicity even without the lock; the lock only serializes concurrent writers.
    """
    ...
    try:
        ...
        if not locked:
            ...
            log(msg)
            # Also route to .vault-meta/hook.log so the user sees the event via
            # wiki-lint (stderr alone is invisible to most callers).
            try:
                META_DIR.mkdir(parents=True, exist_ok=True)

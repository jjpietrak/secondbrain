...
# Locks live inside the ACTIVE VAULT's .vault-meta/, not the code repo. Resolution
# order (adaptation for Second Brain v0.2; CO original defaulted to the script-parent):
#   1. $WIKI_LOCK_VAULT   - test/override hook (handled below; wins over everything)
#   2. $VAULT_ROOT        - exported by `agents.vault_config env`
#   3. `agents.vault_config path` - resolve the active vault on demand
#   4. script-parent fallback (last resort; matches the CO default)

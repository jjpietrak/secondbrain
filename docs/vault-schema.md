...
| ~~objective/topic~~ (RETIRED in v0.2) | **Retired.** The `topic` node type was removed; the subject axis now lives in `wiki/concepts/`, referenced from objective nodes (directions / research_questions / proposals) via a `related:` block of `[[wiki/concepts/<slug>]]` wikilinks.                              | -                 | -              | -              |
...
| objective/direction                  | Research trajectory serving an open research_question. It can represent a reasoning pattern that drives the web-crawl and reasoning agent to fill up a knowledge gap in the vault's domain. Its subject is expressed via `related:` `[[wiki/concepts/<slug>]]` links.    | YES               | Research Agent | Research Agent |

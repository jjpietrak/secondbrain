...
4. Search for the right kanban board - use `$VAULT_ROOT/_CLAUDE.md` board list or search `Boards/`
...
7. Link the task from the relevant project note (an entity with `entity_type: project` under `$VAULT_ROOT/wiki/entities/`) and today's daily note (`$VAULT_ROOT/daily/`)

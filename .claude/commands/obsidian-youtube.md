...
2. Run the Python command from the repo root (`/home/jpietrak/second_brain/`):
   ...
   ```
   (The YouTube backend currently lives at `/home/jpietrak/second_brain/scripts/research/youtube_extract.py`.)
...
5. **Default save behavior: saves automatically.** AI-first note written to `$VAULT_ROOT/research/youtube/YYYY-MM-DD — <video-title-slug>.md`. Frontmatter includes video ID, channel, view counts, etc. for future Dataview queries.
...
8. If the user asks to research something mentioned in the "Worth Following Up On" section, route that to `/obsidian-research [topic]`.

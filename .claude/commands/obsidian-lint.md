...
Execute a structural health pass over the vault at `$VAULT_ROOT/`. Read
`$VAULT_ROOT/_CLAUDE.md` first for vault rules.

Scope: `$VAULT_ROOT/wiki/**` (skip `_template.md` files and `index.md`, `log.md`,
...
Output:
- Write a report to `$VAULT_ROOT/meta/health_report.md` with: generation timestamp,
...
- Append one row to `$VAULT_ROOT/wiki/log.md`:

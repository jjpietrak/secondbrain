...
Sync the vault git repo at `/mnt/c/Obsidian/`. Run these from the code repo or anywhere
(commands use `-C`):

1. Stage everything:
   ```bash
   git -C /mnt/c/Obsidian add -A
   ```
2. Show what changed (for the commit summary):
   ```bash
   git -C /mnt/c/Obsidian status --short
   ```
3. Commit (skip if nothing staged). Use a message summarising the operation that triggered
   ...
   ```bash
   git -C /mnt/c/Obsidian commit -m "vault sync $(date +%Y-%m-%d-%H%M)" || echo "nothing to commit"
   ```
4. Push if a remote is configured:
   ```bash
   git -C /mnt/c/Obsidian remote get-url origin >/dev/null 2>&1 \
     && git -C /mnt/c/Obsidian push 2>&1 \

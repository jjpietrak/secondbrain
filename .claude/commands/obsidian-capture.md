...
3. Search `$VAULT_ROOT/wiki/concepts/` for a related existing note - if found, append to it
4. If new: create `$VAULT_ROOT/wiki/concepts/Title.md` starting from `$VAULT_ROOT/wiki/concepts/_template.md`, ensuring frontmatter has `type, created, updated, sources` (and `tags: [idea]`)
5. Write the idea with any supporting context from the conversation
6. Add a brief mention in today's daily note (`$VAULT_ROOT/daily/`) under an Ideas or Captures section

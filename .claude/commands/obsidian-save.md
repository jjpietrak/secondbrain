...
4. Spawn parallel subagents - one per group - so all note types are handled simultaneously:
   - **People agent**: search for each person, create or update notes in `$VAULT_ROOT/wiki/entities/`, log interactions
   - **Projects agent**: search for each project (a project is an entity with `entity_type: project`), create or update notes in `$VAULT_ROOT/wiki/entities/`
   - **Tasks agent**: parse tasks, add to the right kanban columns
   - **Decisions agent**: find relevant project notes in `$VAULT_ROOT/wiki/entities/`, append to Key Decisions sections
   - **Ideas agent**: search `$VAULT_ROOT/wiki/concepts/` for related notes, create or append
...
5. After all agents complete: update today's daily note in `$VAULT_ROOT/daily/` with links to everything saved
6. Report back: a clean list of what was saved and where

When creating pages under the wiki layer (`$VAULT_ROOT/wiki/entities/`, `$VAULT_ROOT/wiki/concepts/`, `$VAULT_ROOT/wiki/synthesis/`, `$VAULT_ROOT/wiki/sources/`), start from the matching `$VAULT_ROOT/wiki/<folder>/_template.md` and ensure frontmatter has `type, created, updated, sources`.

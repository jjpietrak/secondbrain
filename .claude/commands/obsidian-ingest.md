...
3. Read or fetch the full source content:
   ...
   **For audio files** (.m4a, .mp3, .wav, .ogg, .webm):
   ...
   Save the transcript to `/mnt/c/Obsidian/raw/transcripts/`.
   ...
   **For images/screenshots** (.png, .jpg, .jpeg, .webp):
   ...
   Save the image description to `/mnt/c/Obsidian/raw/articles/` as a markdown summary with context.
...
5. Save the raw source to `/mnt/c/Obsidian/raw/` (immutable - never modify after saving):
   - Create `/mnt/c/Obsidian/raw/articles/YYYY-MM-DD — Source Title.md` (or transcripts/, pdfs/, videos/)
...
6. **REWRITE the vault** - this is the critical step. Don't just create new pages. Rewrite existing ones.
   ...
   Read `/mnt/c/Obsidian/wiki/index.md` first to understand what already exists in the vault. Then spawn parallel subagents:

   - **Entities agent**: for each person/company/tool mentioned:
     - Search `/mnt/c/Obsidian/wiki/entities/` for existing page
     ...
     - If not found: create new entity page (start from `/mnt/c/Obsidian/wiki/entities/_template.md`, frontmatter with `type, created, updated, sources`) with full context
   
   - **Concepts agent**: for each idea/framework/methodology:
     - Search `/mnt/c/Obsidian/wiki/concepts/` for existing or related pages
     ...
     - If not found: create new concept page (start from `/mnt/c/Obsidian/wiki/concepts/_template.md`, frontmatter with `type, created, updated, sources`)
     - If the ingest reveals a PATTERN across multiple existing concepts: create a new synthesis page in `/mnt/c/Obsidian/wiki/synthesis/` (start from `/mnt/c/Obsidian/wiki/synthesis/_template.md`) that connects them (e.g., "Three sources now mention X - this is a trend, not a one-off")
   
   - **Projects agent**: for each project referenced (a project is an entity with `entity_type: project`):
     - Search `/mnt/c/Obsidian/wiki/entities/` for matching project
...
7. Update structural files:
   - REBUILD `/mnt/c/Obsidian/wiki/index.md` - don't just append. Regenerate the sections that changed so descriptions stay current with the rewritten pages.
   - Append to the operation log at `/mnt/c/Obsidian/wiki/log.md`: `## [YYYY-MM-DD] ingest | Source Title (type) — X created, Y rewritten, Z contradictions resolved`

8. Update today's daily note (`/mnt/c/Obsidian/daily/`) with:
   - What was ingested

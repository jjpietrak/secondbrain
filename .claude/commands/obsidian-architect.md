...
5. **Synthesize and write these notes**, each AI-first compliant (start pages from the matching `$VAULT_ROOT/wiki/<folder>/_template.md` and ensure frontmatter has `type, created, updated, sources`):
...
8. Link the overview from the project note and from today's daily note (`$VAULT_ROOT/daily/`). Append a one-line entry to the operation log at `$VAULT_ROOT/wiki/log.md`.

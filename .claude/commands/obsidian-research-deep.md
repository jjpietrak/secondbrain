...
3. **Paid mode** - the script runs a 4-phase pipeline and finishes the work itself:
   ...
   - **Phase 4** - synthesis: Perplexity produces a delta report, the script saves it to `/mnt/c/Obsidian/research/deep/YYYY-MM-DD - <slug>.md`, then emits a JSON payload between `<<<RESEARCH_DEEP_PROPAGATION_PAYLOAD>>>` markers.
...
4. **Free mode** - the script does Phase 1 (vault scan) plus free-source aggregation and prints a JSON block with `"mode": "free-sources-deep"`, containing `vault_baseline_notes` (path, score, excerpt of what the vault already knew), `sources` (fresh external results), `stats`, `warnings`, and an `instruction`. YOU are the synthesizer:
   ...
   - Save it yourself to `/mnt/c/Obsidian/research/deep/YYYY-MM-DD - <slug>.md`, starting from `/mnt/c/Obsidian/wiki/synthesis/_template.md` and following `/home/jpietrak/second_brain/skills/references/ai-first-rules.md` (preamble; frontmatter with `type: research-deep`, `ai-first: true`, `created`, `updated`, `vault-baseline-notes`, and a `sources` list of every result URL verbatim).
...
5. **Propagation (both modes):**
   ...
   - Link the new research note from today's daily note (`/mnt/c/Obsidian/daily/`).

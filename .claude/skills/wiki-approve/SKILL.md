...
name: wiki-approve
description: >
  Read back ticked approve/reject checkboxes from a nightly_report and apply them to the
  ingest_index, then fetch and ingest each approved source. Triggers on: "wiki-approve",
  "/wiki-approve", "process approvals", "apply report decisions", "ingest approved sources".
...
> ## For future Claude
> The user ticked boxes in a nightly_report and you are now turning those ticks into
> real ingest_index status flips, raw-file fetches, and wiki pages. Three outcomes:
> approve -> fetch + wiki-ingest, reject -> sticky, conflict -> skip + surface.
> ai-first: true
...
- **Manual** (user runs `/wiki-approve` or asks "process approvals"): after the user opens
  the latest `meta/nightly_report/<date>.md` in Obsidian, ticks approve/reject boxes for
  the candidates, and invokes the command.
...
REPORT="$(ls -1t "$VAULT_ROOT/meta/nightly_report/"*.md 2>/dev/null | head -1)"
```
...
Show the user EXACTLY what will be approved, rejected, and flagged as conflicts BEFORE
applying. Print the JSON plan and ask for confirmation if there are conflicts.
...
Add `--apply` to flip statuses in the ingest_index:
...
`scripts/report_approve.py::parse_report(md_text)` is the single source of truth for
checkbox parsing. Both the manual `/wiki-approve` command (this skill) and the Phase-4
nightly auto-read import and call it directly. Do NOT duplicate the parsing logic.

---

## What not to do

- Do not apply without the dry-run preview step (user needs to see conflicts first).
